...
| 2026-08-19 | Add an explicit all-day/time flag to `Item` in Room v4. | The existing timestamp remains the local date/time value; the flag preserves the distinction between an all-day date and midnight, and migration keeps all prior dates all-day. |
| 2026-08-19 | Reuse `AlarmClock.ACTION_SET_ALARM` for dated-item alarm shortcuts. | The action delegates to Samsung Clock, retains its next-24-hours validation, and does not activate the dormant Room alarm table or introduce background scheduling. |
